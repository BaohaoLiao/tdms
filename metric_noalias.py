...
def reformat_eval_df(df):
    ...
    for index, row in df.iterrows():
        for name in df.columns[1:-1]:
            row_id = str(row["study_id"]) + "_" + name
            row_ids.append(row_id)
            if row[name] == "Normal/Mild":
                normal_milds.append(1)
                moderates.append(0)
                severes.append(0)
                sample_weights.append(1)
            elif row[name] == "Moderate":
                normal_milds.append(0)
                moderates.append(1)
                severes.append(0)
                sample_weights.append(2)
            elif row[name] == "Severe":
                normal_milds.append(0)
                moderates.append(0)
                severes.append(1)
                sample_weights.append(4)
            else:
                normal_milds.append(1/3)
                moderates.append(1/3)
                severes.append(1/3)
